...
// (3) 조건 : 만약에 총점이 90점이상   삼항연산자 vs 조건문
// (4) 참 : '성공' 출력한다.
// (5) 거짓 : '실패' 출력한다.
// 방법1 : 삼항연산자
...
console.log( result1) ;
// 방법2 : 조건문
if( sum >= 90 ) {   // 1. 만약에 (if) 총점(sum)dl 90점이상(>=) 이면
...
//else if( pwdLen >=8 ){ console.log("보안 등급 : 보통"); }
//else { 
...
*/
/*let seatList = [ 'O', 'X', 'O' ]; //(1) 배열이란? 여러개의 자료드을 하나의 자료로 [ ]묶음
let seat = prompt("좌석번호 선택(0~1) :" ); // (2) 인덱스일나 ? 배열에 저장된 자료들의 순서번호 *0번시작*
if ( seatList[seat] =='O' ){ //만약에 좌석목록(seatList)에서 선택(입력받은)한 (시트/인덱스)가 'O'이면
    console.log( '예약불가');
//}else if( seatList[ seat ] == 'X'){ // 2번째 조건은 }
...
else if( gamePoint >=700 && gamePoint < 900){ console.log( 'B급 경품'); }
else if( gamePoint >= 500 && < 700 ){ console.log( 'C급 경품'); }
