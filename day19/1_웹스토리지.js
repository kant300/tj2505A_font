...
[ 웹스토리지 ]
    ...
    2. 사용법
        ...
        (4) sessopmStprage.clear();
        * 속성명/key는 무조건 문자만 가능
        * value는 객체와배열 타입 저장 불가능. 리터럴(문자열, 숫자, 불리언) 자료만 가능

[ JSON ] : 자바스크립트 객체 문법의 형식, javaScript Object Notaion        
    *자바 객체를 문자열타입 이지만 객체형식/모양으로 표현한것 
    - 3 : 숫자타입 , 숫자모양                       vs "3" : 문자열타입 , 숫자모양
    - { age : 40 } : 객체타입 , 객체모양            vs "{ age : 40 }" : 문자열타입 , 객체모양
    = 왜 ??? 인터넷상의 데이터통신하는 http는 객체지향이 아니다. 변환이 필요하다. 문자열과 바이트 전송만 가능
...
console.log( localStorage.getItem( 'phone' ) );     // null 존재하지 않는 속성명호출시
