...
[ for ]
    ...
    2. 문법
        (1) 초기값 : 반복할때 반복의 카운터/횟수 체크하는 변수의 처음값
    ...
    4. 배열과 반복문 관계 : 인덱스
        (1) 배열의 인덱스는 순차적인 순서번호가 존재한다. 인덱스란 ? 자료가 저장된 순서번호*0번시작*
        (2) 배열의 길이(자료개수, 1시작) : 변수명.length
        (3) 배열의 마직막 인덱스 ( 0시작 ) : 변수명.length - 1
            회원이 총 3명이면, 인덱스튼 0 1 2, 마지막 인덱스 : 2
*/
// [1] 배열의 데이터
/*
...
// [반복문사용]  : 패턴 : 0부터 2까지 1씩 증가하는 인덱스!!
for(let index = 0 ; index <= 2 ; index++ ){
    console.log( 과일상자 [ index ] );
}
// [반복문사용]  : 패턴 : 0부터 2까지(마지막까지/인덱스) 1씩 증가하는 인덱스!!
...
for( let count = 1 ; count <= 3 ; count++ ){
    let data1 = prompt( "data1 : " ); //count는 1부터 3까지 1씩 증가 <3 번반복>
    dataList.push( data1);        
}
