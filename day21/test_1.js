...
function getProducts() {
    // 'products'키로 저장된 데이터를 로컬스토리지에 가져옵니다.
    let products = localStorage.getItem('products'); console.log(products);
    // 만약 가져온 데이터가 없으면(null), 빈배열로 초기화
    if (products == null) {
        products = [];
    } else {
        //데이터가 있다면 , JSON 문자열을 실제 자바스크립트 배열/객체로 변환
        products = JSON.parse(products);
    }
    // 처리된 products 배열을 반환한다.
    return products;
}

// 주어진 products 배열을 로컬 스토리지에 저장하는 함수
function setProducts(products) {
    //'products'배열을 JSON문자열 형태로 변환하여 로컬스토리지에 저장
...
let pcode = products.length == 0 ? 1 : products[products.length - 1].pcode + 1
    ...
    pcode: pcode,
    pname: pname,
    pprice: Number(pprice), // 가격은 숫자형으로 변환
    pcontent: pcontent,
...
productPrint(); //페이지가 처음 로드될때 제품목록을 한번 출력
function productPrint() {
    ...
    for (let index = 0; index < products.length; index++) {
        const product = products[index]; // 현재 순회 중인 제품 객체입니다.
...
function productDelete(pcode){
    ...
    // products 배열의 모든 요소를 순회하여 삭제할 제품을 찾습니다. for
    for( let index = 0 ; index < products.length ; index++ ){
        // 만약 현재 순회 중인 제품의 pcode가 삭제할 pcode와 일치하면
        if( products[index].pcode == pcode){
            // 배열에서 해당 요소를 제거합니다. (index번째 인덱스에서 1개 삭제)
